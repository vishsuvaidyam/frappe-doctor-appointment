<template>
    <div class="inset-0 flex items-center justify-center bg-opacity-50 bg-gradient-to-b from-teal-200 to-white mt-10 py-10">
        <div v-for="doctor in doctorDetails" :key="doctor.doctor.full_name" class="max-w-4xl w-full bg-white p-8 rounded-lg shadow-md">
            ...
            <form class="space-y-6" @submit.prevent="submitForm(doctor)">
                <!-- Name Input Fields -->
                <div>
                ...
                <div>
                    ...
                    <input v-model="patient.email" type="email" :class="{ 'border-red-500': emailError }"
                        @input="validateEmail" placeholder="example@example.com"
                ...
                <div>
                    ...
                    <small v-if="ageError" class="block mt-1 text-red-500">{{ ageErrorMessage }}</small>
                </div>
...
const submitForm = async (doctor) => {
    ...
    try {
        const response = await axios.post("/api/method/appointments_management.controllers.api.appointment_data", {
            ...
            doctor_image: doctor.doctor.doctor_image,
            address: doctor.doctor.address,
            doctor_fees: doctor.doctor.doctor_fee,
            patient_age: patient.age,
            gender: patient.gender,
            email: patient.email,
            datetime: route.params.formattedDateTime,
        });
        if (response) {
